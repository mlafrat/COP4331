...
function Dashboard() {
  ...
  return(
    <div className="review-wrapper">
        ...
        <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={3} columns={{ xs: 12, sm: 6, md: 4 }} sx={{justifyContent:'center', flexDirection:'column', paddingLeft:'1000px'}}>
                {microwaves.map((microwave, index) => (
                    <Grid item key={index}>
                        <Card>
                            <CardContent sx={{display:'block'}}>
                                ...
                                <p style={{inlineSize:'350px', textAlign:'center', paddingLeft:'0px'}}>{microwave.location_building}</p>
                                </div>
                                <div className="subtitle">
                                    <p style={{fontWeight:'700', textAlign:'center'}}>Location Description: </p>
                                    <p style={{inlineSize:'350px', textAlign:'center', paddingLeft:'0px'}}>{microwave.location_description}</p>
                                </div>
                                <div className="subtitle">
                                    <p style={{fontWeight:'700', textAlign:'center'}}>Rating: </p>
                                    <p style={{inlineSize:'350px', textAlign:'center', paddingLeft:'0px'}}>{microwave.total_rating/microwave.users_rated}</p>
                                </div>                                    
                            </CardContent>
                            <CardActions sx={{justifyContent:'center'}}>
                                <Button variant='contained' onClick={() => handleView(microwave.microwave_id)}>View Reviews</Button>
                            </CardActions>
                        </Card>
                    </Grid>
                ))}
